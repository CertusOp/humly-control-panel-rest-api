...
| Name              | Type   | Comment |
| ----------------- | -------| ------- |
| `equipment`       | String | This parameter should be used to report broken or fixed equipment for given room. Parameter should be provided as “stringified” representation of JSON object. Format: `{`<br>`  "a": "a"`<br>`}`<br>*{<br>  "lights": true,<br>  "projector": false,<br>  "computer": true<br>}*<br>Room equipment will be reported as fixed by specifying “true” as their value or broken if “false” is provided. List only equipment that should change state from broken to fixed, or vice versa. |
| `customEquipment` | String | This parameter should be used to report broken or ```{\n"a": "a"\r}``` fixed custom equipment for given room. Parameter should be provided as “stringified” representation of JSON object. Format: <br>>[<br>&nbsp;&nbsp;{<br>    "name": "Gadget",<br>    "isChecked": false<br>  }<br>]<br>Room custom equipment will be reported as fixed by specifying “true” as value of “isChecked” parameter or broken if “false” is provided. List only custom equipment that should change state from broken to fixed, or vice versa. |
